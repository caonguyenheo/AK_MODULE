...
static int cmd_sd_unmount(void)
{
	...
	int ret = ak_unmount_fs(DEV_MMCBLOCK, 0, "");
	if (0 == ret)
 ...
 *****************************************/
static void *init_video_in(int main_width, int main_height, int sensor_fps,int is_day)
{
	void *handle;
 ...
 * @return void
 *****************************************/
static void get_frames(void *handle, int framecount, int frame_interval,  unsigned char *save, int *save_len)
{
...
void cmd_vi_test(int argc, char **args)  
{
	...
	handle = init_video_in(width, height, sensor_fps,is_day);
	if (NULL == handle) {
		goto finish_free;
	}
	ak_vi_capture_on(handle);
	get_frames(handle, framecount, frame_interval, yuv, &yuv_len);
