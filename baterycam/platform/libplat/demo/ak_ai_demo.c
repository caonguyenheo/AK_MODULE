...
void cmd_ai_test(int argc, char **args)
{
	...
	char* p_end_tmp = NULL;
	char* p_end_tmp_1 = NULL;
	
	total_ts = strtol(args[0], &p_end_tmp, 10 ) * 1000;
	samplerate = strtol(args[1], &p_end_tmp_1, 10 ) * 1000;
	if(argc == 0)
	{	
		total_ts = 30 * 1000;	
		samplerate = 8 * 1000;
	...
	{
		if(total_ts > 0 && *p_end_tmp == '\0' && *p_end_tmp_1 == '\0')
		{
			samplerate = 8 * 1000;
		}
		else
	...
	{
		if(total_ts < 0 || *p_end_tmp != '\0')
		{
			ak_print_error_ex("Input command error:total_ts must large than 0\n");    	
	    	ak_print_normal("%s\n",help[1]);
			return ;
		}
		else if((samplerate != 8 * 1000) && (samplerate != 16 * 1000) || *p_end_tmp_1 != '\0')
		{
