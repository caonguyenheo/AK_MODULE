...
static ak_sem_t  sem_socket_stop;
...
int tcp_client_connect(void *args)
{
	...
	else
	{
		...
		else
		{
            ...
            ak_print_normal("socket conn ok\n");
...
int tcp_recv_task(void *args)
{
    ...
    while(1) {
        ...
        while(1) {
            ...
            {
                ak_print_error("tcp socket recv data exit\n");
                ak_thread_sem_post(&sem_socket_stop);
                break;
            } else {
...
//		goto exit1;
	...
	check_sum = 0;
   	while (sys_video_start) {
        ...
        int send_len = send_data(send_buf, stream.len + sizeof(VIDEO_STREAM_HEADER) + sizeof(VIDEO_STREAM_END));
		if( send_len < 0)
		{
        ...
        //ak_print_normal("video send type %d ts %lu id %d len %d\n",p_stream->frame_type,(long)stream.ts,id,send_len);
        id++;
...
void* camview_main(int argc, char **args)
{
    ...
    ret = ak_thread_sem_init(&sem_socket_stop, 0);
	...
	while(1)
	{
        ...
        if(g_tcp_socket_id >= 0) {
            ...
            shutdown(g_tcp_socket_id, SHUT_RDWR);
            //ak_sleep_ms(1000);
            ak_thread_sem_wait(&sem_socket_stop);
            ak_print_normal("close socket!\n");
            closesocket(g_tcp_socket_id);
            g_tcp_socket_id = -1;
        }
		ak_sleep_ms(100);
        ak_print_normal("socket conn...\n");
        tcp_client_connect((void*)&sockaddr);
