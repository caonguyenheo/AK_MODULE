 ...
 */
void cmd_wifi_sta(int argc, char **args)
{
	char *essid = args[1];
...
void cmd_wifi_ap(int argc, char **args)
{
...
static void cmd_wifi_ssv(int argc, char **args)
{
    int i = 0;
    static char cmd_wifi_str[128];
    //ak_print_normal("%s",help_wifi_ssv[0]);
    memset(cmd_wifi_str, 0, 128);
    for (i = 0;i < argc; i++)
    {
        strcat(cmd_wifi_str, args[i]);
        strcat(cmd_wifi_str, " ");
    }
    if (strlen(cmd_wifi_str))
    {
        //Cli_RunCmd(cmd_wifi_str);
    }
}
...
int wifi_demo_init()
{
	...
	//cmd_register("wifi", cmd_wifi_ssv, help_wifi_ssv);
	return 0;
}
